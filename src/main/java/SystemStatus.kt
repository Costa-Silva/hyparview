    ...
    init {
        while (true) {
            ...
            when(option) {
                ...
                "1.2" -> printlnErr(pvWrapper.activeView.map { it.path().name() })
                "1.3" -> printlnErr(pvWrapper.passiveView.map { it.path().name() })
                "1.4" -> printlnErr(pvWrapper.passiveActiveView.map { it.path().name() })
